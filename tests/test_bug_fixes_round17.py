...
class TestR17AUTH1TimingSafeComparison:
    ...
    def test_device_key_uses_secrets_compare_digest(self):
        ...
        if func_match:
            ...
            has_timing_safe = (
                "secrets.compare_digest" in func_source
                or "hmac.compare_digest" in func_source
                or "compare_digest" in func_source
            )
...
class TestR17AUTH2RefreshTokenRotation:
    ...
    def test_refresh_invalidates_old_token(self):
        ...
        if refresh_match:
            ...
            has_blacklist = (
                "blacklist" in refresh_source.lower()
                or "token_blacklist" in refresh_source
                or "revoke" in refresh_source.lower()
            )
            ...
            assert has_blacklist or has_awareness, (
                "Auth refresh should invalidate old refresh token to prevent token reuse attacks."
            )
...
class TestR17CRYPTO1TagChecksumValidation:
    ...
    def test_tag_confirm_validates_checksum(self):
        ...
        if confirm_match:
            ...
            has_checksum_check = (
                "checksum" in confirm_source.lower()
                or "hmac" in confirm_source.lower()
                or "verify" in confirm_source.lower()
                or "sig" in confirm_source.lower()
            )
            ...
            assert has_checksum_check or accesses_checksum, (
                "Tag confirmation should validate checksum/HMAC to prevent tag forgery."
            )
...
class TestR17SESS1JWTClaims:
    ...
    def test_jwt_includes_issuer(self):
        ...
        decode_match = re.search(r"def decode_token.*?(?=\ndef |\nclass |\Z)", source, re.DOTALL)
...
class TestR17SESS2BlacklistConsistency:
    ...
    def test_blacklist_has_redis_backend(self):
        ...
        has_fallback_warning = "fallback" in source.lower() or "memory" in source.lower()
...
class TestR17API1SensitiveLogging:
    ...
    def test_whatsapp_client_doesnt_log_token(self):
        ...
        logs_token = re.search(r"logger\.\w+\([^)]*access_token[^)]*\)", source, re.IGNORECASE)
        logs_auth = re.search(r"logger\.\w+\([^)]*authorization[^)]*\)", source, re.IGNORECASE)
...
class TestR17API2SESLogging:
    ...
    def test_ses_client_doesnt_log_credentials(self):
        ...
        logs_key = re.search(r"logger\.\w+\([^)]*aws.*key[^)]*\)", source, re.IGNORECASE)
        logs_secret = re.search(r"logger\.\w+\([^)]*secret[^)]*\)", source, re.IGNORECASE)

        assert not logs_key and not logs_secret, "SES client should not log AWS credentials."
...
class TestR17DB1GuardianOwnership:
    ...
    def test_consent_service_validates_ownership(self):
        ...
        has_ownership_check = (
            "guardian" in source
            and "student" in source
            and ("if" in source or "assert" in source or "raise" in source)
        )

        # Check for relationship validation
        validates_relationship = (
            "guardians" in source or "students" in source or "belongs" in source.lower()
        )

        assert has_ownership_check or validates_relationship, (
            "Consent service should validate guardian owns the student before updating preferences."
        )
...
class TestR17DB2MassAssignment:
    ...
    def test_user_repo_uses_explicit_fields(self):
        ...
        if update_match:
            ...
            has_explicit = (
                ".email" in update_source
                or ".full_name" in update_source
                or ".is_active" in update_source
            )
...
class TestR17FE2TokenStorage:
    ...
    def test_api_base_uses_session_storage(self):
        ...
        if uses_local_storage:
            ...
            local_for_tokens = re.search(r'localStorage\.\w+\([\'"].*token', content, re.IGNORECASE)
...
class TestR17FE3InlineHandlers:
    ...
    def test_components_no_inline_onclick(self):
        ...
        if has_inline_onclick or has_inline_onsubmit:
            ...
            has_delegation = "delegate" in content.lower() or "data-action" in content
...
class TestR17CONFIG1SecretsValidation:
    ...
    def test_config_validates_device_api_key(self):
        ...
        assert has_device_key, "Config should define and validate DEVICE_API_KEY."
...
class TestR17CONFIG2RedisAuth:
    ...
    def test_config_supports_redis_password(self):
        ...
        ("password" in source.lower() or "auth" in source.lower() or "redis://" in source.lower())

        assert has_redis_url, "Config should support REDIS_URL which can include authentication."
...
class TestR17AUDIT1SecurityLogging:
    ...
    def test_auth_api_logs_failures(self):
        ...
        has_auth_logging = "login" in source.lower() and (
            "warning" in source
            or "info" in source
            or "error" in source
            or "audit" in source.lower()
        )
...
class TestR17AUDIT2AdminAudit:
    ...
    def test_webauthn_admin_logs_actions(self):
        ...
        ("admin" in source.lower() and ("logger." in source or "audit" in source.lower()))

        assert has_logger, "WebAuthn API should have logging for admin credential operations."
...
class TestR17RATE1AuthRateLimiting:
    ...
    def test_auth_login_has_rate_limit(self):
        ...
        assert has_limiter, (
            "Auth login endpoint should have rate limiting to prevent brute force attacks."
        )
...
class TestR17VALID1PathValidation:
    ...
    def test_student_id_has_ge_validation(self):
        ...
        assert uses_int_type, "Attendance API should validate student_id path parameter."

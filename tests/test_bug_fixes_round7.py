...
class TestR7A6PathParameterValidation:
    ...
    def test_schedule_endpoints_validate_positive_ids(self):
        ...
        has_path_validation = "Path(" in source and ("ge=1" in source or "gt=0" in source)

        # O al menos debería haber validación manual
        has_manual_validation = (
            "if course_id < 1" in source
            or "if schedule_id < 1" in source
            or "if exception_id < 1" in source
            or "<= 0" in source
        )
    ...
    def test_list_alerts_has_default_limit(self):
        ...
        if limit_param:
            ...
            has_safe_default = (
                ...
                and limit_param.default is not None
                and isinstance(limit_param.default, int)
                and limit_param.default > 0
            )
...
class TestR7S5BroadcastRedisCleanup:
    ...
    def test_broadcast_service_has_cleanup(self):
        ...
        has_cleanup = (
            hasattr(BroadcastService, "__del__")
            or hasattr(BroadcastService, "close")
            or hasattr(BroadcastService, "__enter__")  # context manager
        )
        ...
        if not has_cleanup:
            ...
            has_cleanup_awareness = (
                ...
                or "close" in source.lower()
                or "context manager" in source.lower()
            )
...
class TestR7A5BroadcastRateLimiting:
    ...
    def test_broadcast_endpoints_have_rate_limiting(self):
        ...
        has_limiter = (
            "@limiter" in source or "RateLimiter" in source or "rate_limit" in source.lower()
        )

        # Los endpoints de broadcast son críticos y deben tener rate limiting
        # para prevenir spam masivo
        assert has_limiter, "R7-A5 Bug: Broadcast endpoints sin rate limiting"
...
class TestR7A1AttendancePagination:
    ...
    def test_list_events_has_pagination(self):
        ...
        has_pagination = (
            "limit" in source
            and ("offset" in source or "skip" in source)
            or "page" in source
            or ".limit(" in source
        )

        assert has_pagination, "R7-A1 Bug: Attendance list endpoint sin paginación"
...
class TestR7S3RedisErrorLogging:
    ...
    def test_notification_service_logs_redis_errors(self):
        ...
        has_error_logging = (
            "logger.error" in source or "logger.exception" in source or "logging.error" in source
        )
...
class TestR7B2WebAuthnDeleteAll:
    ...
    def test_webauthn_delete_all_uses_bulk_delete(self):
        ...
        if "delete_all_for_student" in source:
            ...
            has_n_plus_one = (
                "for cred in" in method_section and "session.delete(cred)" in method_section
            )

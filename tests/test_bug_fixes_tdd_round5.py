...
class TestBugR5_4_MissingRadixInTeacherPWA:
    ...
    def test_teacher_pwa_uses_parseint_with_radix(self):
        ...
        issues = []
        for root, _dirs, files in os.walk(pwa_dir):
            for file in files:
                if file.endswith(".js"):
        ...
        assert len(issues) == 0, f"parseInt without radix found in teacher-pwa: {issues}"
...
class TestBugR5_5_NotificationPayloadTypeCheck:
    ...
    def test_notification_export_handles_payload_types(self):
        ...
        assert has_isinstance, "Notification export should check isinstance(item.payload, dict)"

...
class TestR4S4DatetimeConsistency:
    ...
    def test_webauthn_uses_timezone_aware_everywhere(self):
        ...
        aware_count = source.count('datetime.now(timezone.utc)')
...
class TestR4F5StateUpdateStudents:
    ...
    def test_update_students_pattern_validates_empty(self):
        ...
        if state_path.exists():
            ...
            has_empty_check = (
                'serverStudents.length === 0' in source or
                'serverStudents.length == 0' in source or
                '!serverStudents || serverStudents.length' in source or
                'serverStudents?.length' in source
            )

            assert has_empty_check, \
                "R4-F5 Bug: updateStudents() no valida lista vacía del servidor"
    ...
    def test_audio_context_is_closed(self):
        ...
        if biometric_path.exists():
            ...
            if 'AudioContext' in source:
                ...
                assert has_close, \
                    "R4-F7 Bug: AudioContext creado pero nunca cerrado (memory leak)"
...
class TestR4F10LocalStorageQuota:
    ...
    def test_persist_handles_quota_error(self):
        ...
        if state_path.exists():
            ...
            has_quota_handling = (
                'QuotaExceededError' in source or
                ('try' in source and 'localStorage.setItem' in source and 'catch' in source)
            )

            assert has_quota_handling, \
                "R4-F10 Bug: persist() no maneja QuotaExceededError"
...
class TestR4F2SyncIntervalsCleanup:
    ...
    def test_sync_intervals_cleanup_on_unload(self):
        ...
        if sync_path.exists():
            ...
            has_unload_cleanup = (
                'beforeunload' in source or
                'pagehide' in source or
                'unload' in source
            )

            assert has_unload_cleanup, \
                "R4-F2 Bug: Sync intervals no se limpian en page unload"
...
class TestR4F1UIIntervalCleanup:
    ...
    def test_ui_header_interval_has_reference(self):
        ...
        if ui_path.exists():
            ...
            has_interval_ref = (
                '_headerInterval' in source or
                'headerIntervalId' in source or
                'UI.interval' in source
            )
...
class TestR4S1IPSpoofing:
    ...
    def test_forwarded_for_validates_trusted_proxy(self):
        ...
        has_proxy_validation = (
            'trusted_proxy' in source.lower() or
            'proxy_whitelist' in source.lower() or
            'verify_proxy' in source.lower() or
            # O comentario documentando el riesgo
            'X-Forwarded-For' in source and 'trust' in source.lower()
        )
...
class TestR4L1EvidencePreferenceConsistency:
    ...
    def test_attendance_service_uses_evidence_preference(self):
        ...
        uses_new_pref = (
            'evidence_preference' in source or
            'effective_evidence_preference' in source
        )

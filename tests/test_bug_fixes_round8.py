...
class TestR8C4OpenAPIProduction:
    ...
    def test_openapi_disabled_in_production(self):
        ...
        has_conditional = (
            "openapi_url" in source
            and "production" in source.lower()
            or "docs_url" in source
            and "production" in source.lower()
            or "openapi_url=None" in source
        )

        # O al menos debe haber comentario sobre el riesgo
        has_awareness = "production" in source.lower() and (
            "docs" in source.lower() or "openapi" in source.lower()
        )
    ...
    def test_health_check_verifies_database(self):
        ...
        if "def healthcheck" in source or "async def healthcheck" in source:
            ...
            has_db_check = (
                ...
                or "session" in source.lower()
                or "execute" in source
                or "select 1" in source.lower()
            )
...
class TestR8C7LoggingBacktrace:
    ...
    def test_logging_backtrace_conditional(self):
        ...
        has_conditional = (
            ...
            and "backtrace" in source
            or "backtrace=False" in source
            or "settings." in source
            and "backtrace" in source
        )
    ...
    def test_cors_credentials_not_with_wildcard(self):
        ...
        if has_wildcard and has_credentials:
            ...
            has_safety = (
                ...
                or "if" in source
                and "credentials" in source.lower()
            )
    ...
    def test_login_request_validates_email_format(self):
        ...
        try:
            ...
            has_email_validation = (
                "EmailStr" in source or "@validator" in source or "@field_validator" in source
            )
...
class TestR8V4ContactTypeEnum:
    ...
    def test_guardian_contact_type_uses_enum(self):
        ...
        uses_enum = "ContactType" in source or "Literal[" in source or "Enum" in source

        assert uses_enum, "R8-V4 Bug: GuardianContact.type acepta cualquier string"
...
class TestR8V6AttendanceTypeEnum:
    ...
    def test_attendance_event_summary_type_uses_enum(self):
        ...
        uses_enum = (
            "AttendanceType" in source
            or "Literal[" in source
            and ("IN" in source or "OUT" in source)
            or "EventType" in source
        )

        assert uses_enum, "R8-V6 Bug: AttendanceEventSummary.type acepta cualquier string"
...
class TestR8F5FetchTimeout:
    ...
    def test_api_base_fetch_has_timeout(self):
        ...
        if api_base_path.exists():
            ...
            has_timeout = (
                "AbortController" in source
                or "signal" in source
                or "Promise.race" in source

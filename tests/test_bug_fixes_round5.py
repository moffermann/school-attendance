...
class TestR5D1OpenAPIDocs:
    ...
    def test_openapi_disabled_in_production(self):
        ...
        if settings.app_env == "production":
            assert app.openapi_url is None, "R5-D1 Bug: OpenAPI docs expuestas en producción"
...
class TestR5D2HSTSHeader:
    ...
    def test_hsts_enabled_in_production_code(self):
        ...
        has_hsts_with_production = (
            ...
            and "production" in source
            and "response.headers" in source
        )
...
class TestR5A1StatusFilterValidation:
    ...
    def test_alert_status_uses_enum(self):
        ...
        uses_enum = (
            "AlertStatus" in source
            or "StatusFilter" in source
            or "Literal[" in source
            and "PENDING" in source
        )

        assert uses_enum, "R5-A1 Bug: status_filter acepta cualquier string sin validación"
...
class TestR5A7CSVInjection:
    ...
    def test_absences_export_sanitizes_csv(self):
        ...
        has_sanitization = (
            "_sanitize_csv" in source
            or "sanitize" in source.lower()
            or "=CMD" in source  # Checking for formula protection
            or "formula" in source.lower()
        )

        assert has_sanitization, "R5-A7 Bug: Export de absences no sanitiza valores CSV"
...
class TestR5A9NotificationsCSVInjection:
    ...
    def test_notifications_export_sanitizes_csv(self):
        ...
        # Debe tener sanitización de CSV
        has_sanitization = "_sanitize_csv" in source or "sanitize" in source.lower()

        assert has_sanitization, "R5-A9 Bug: Export de notifications no sanitiza valores CSV"
...
class TestR5B1DatetimeDefaults:
    ...
    def test_notification_model_uses_timezone_aware_default(self):
        ...
        assert not uses_naive, "R5-B1 Bug: Notification usa datetime.utcnow naive como default"
    ...
    def test_tag_model_uses_timezone_aware_default(self):
        ...
        assert not uses_naive, "R5-B1 Bug: Tag usa datetime.utcnow naive como default"
...
class TestR5D10LogLevel:
    ...
    def test_logging_disables_diagnose_in_production(self):
        ...
        has_diagnose_conditional = (
            "diagnose=False" in source
            or ("diagnose" in source and "production" in source.lower())
            or "diagnose" not in source  # Si no usa diagnose, está bien
        )

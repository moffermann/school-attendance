...
class TestR6W1EmailRetryLogic:
    ...
    def test_send_email_has_retry_logic(self):
        ...
        has_retry = (
            "TRANSIENT_ERRORS" in source
            or "MAX_RETRIES" in source
            or ("retry" in source.lower() and "transient" in source.lower())
        )

        assert has_retry, "R6-W1 Bug: send_email no tiene retry logic para errores transitorios"
...
class TestR6W2EmailHTMLEscape:
    ...
    def test_send_email_escapes_html_in_variables(self):
        ...
        has_escape = (
            "escape" in source.lower()
            or "html.escape" in source
            or "markupsafe" in source.lower()
            or "sanitize" in source.lower()
        )

        assert has_escape, "R6-W2 Bug: send_email no escapa HTML en variables (XSS vulnerability)"
...
class TestR6M5CourseNameIndex:
    ...
    def test_course_name_should_be_indexed(self):
        ...
        if name_column is not None:
            ...
            assert has_index, "R6-M5 Info: Course.name no tiene índice (performance)"
...
class TestR6W3ConfigurableRetries:
    ...
    def test_max_retries_is_configurable(self):
        ...
        (
            "settings.max_retries" in source.lower()
            or "settings.notification_retries" in source.lower()
            or "config.max_retries" in source.lower()
        )

        # Por ahora solo verificamos que está definido consistentemente
        has_constant = "MAX_RETRIES" in source

        assert has_constant, "R6-W3: MAX_RETRIES debería ser configurable via settings"
...
class TestR6M7NotificationPrefsValidation:
    ...
    def test_notification_prefs_accepts_any_structure(self):
        ...
        with pytest.raises(ValidationError):
            GuardianPreferencesUpdate(preferences={"INGRESO_OK": "not_a_dict"})
...
class TestR6M8NullableFieldsHandling:
    ...
    def test_notification_payload_handles_none(self):
        ...
        has_null_check = (
            "payload or" in source
            or "if notification.payload" in source  # R6-M8 fix: correct pattern
            or "payload and" in source
            or "notification.payload.get" not in source  # Ya no hace .get() directo
        )

        assert has_null_check, "R6-M8 Bug: _map_notification no maneja payload=None"
...
class TestR6W4WorkerSessionManagement:
    ...
    def test_workers_use_context_manager(self):
        ...
        from app.workers.jobs import send_email, send_whatsapp
        ...
        assert whatsapp_ok, "R6-W4: send_whatsapp no usa context manager para session"
        assert email_ok, "R6-W4: send_email no usa context manager para session"
...
class TestR6M9AttendanceEventDatetime:
    ...
    def test_attendance_event_occurred_at_timezone_handling(self):
        ...
        if occurred_at_col is not None:
            ...
            assert has_timezone, "R6-M9: occurred_at debe ser DateTime(timezone=True)"
...
class TestR6T2AssertionMessages:
    ...
    def test_test_assertions_have_messages(self):
        ...
        if files_with_bare_asserts:
            ", ".join(f"{f}({c})" for f, c in files_with_bare_asserts)

...
Views.parentAbsences = function() {
        ...
        <form id="absence-form">
          ...
          <div class="form-group">
            ...
            <div class="form-error" style="color: var(--color-gray-500); margin-top: 0.25rem;">
              Nota: por ahora solo se registra el nombre del archivo como referencia.
            </div>
          </div>
  ...
  Views.parentAbsences.submitRequest = async function() {
    ...
    const absence = {
      ...
      type: document.getElementById('absence-type').value.toUpperCase(),
    ...
    try {
      await State.submitAbsenceRequest(absence);
      Components.showToast('Solicitud enviada exitosamente', 'success');
      form.reset();
      setTimeout(() => Views.parentAbsences(), 400);
    } catch (error) {
      console.error('No se pudo enviar la solicitud de ausencia', error);
      Components.showToast(error?.message || 'No fue posible registrar la solicitud', 'error');
      if (submitButton) {

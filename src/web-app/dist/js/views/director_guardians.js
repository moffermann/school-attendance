...
Views.directorGuardians = async function() {
        ...
        <main class="flex-1 flex flex-col overflow-hidden">
          ...
          <header class="h-20 bg-white dark:bg-card-dark border-b border-slate-200 dark:border-border-dark flex items-center justify-between px-8 z-10">
            <div class="flex items-center gap-4">
              <button class="desktop-hidden p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg" onclick="Components.toggleDirectorSidebar()">
      ...
      const actionButtons = isDeleted
        ...
        : `
            <div class="flex items-center justify-end gap-1">
              ...
              <!-- Reenviar Invitación -->
              <button onclick="Views.directorGuardians.resendInvitation(${guardian.id})"
                      class="p-1.5 text-slate-400 hover:text-teal-600 hover:bg-teal-50
                             dark:hover:text-teal-400 dark:hover:bg-teal-900/30 rounded transition-colors"
                      title="Reenviar Invitación">
                <span class="material-icons-round text-[20px]">mail</span>
              </button>
  ...
  Views.directorGuardians.resendInvitation = async function(guardianId) {
    const guardian = State.getGuardian(guardianId);
    if (!guardian) {
      Components.showToast('Apoderado no encontrado', 'error');
      return;
    ...
    try {
      const response = await API.request(`/guardians/${guardianId}/resend-invitation`, {
        method: 'POST',
      });

      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.detail || 'Error al reenviar invitación');
      }

      Components.showToast('Invitación reenviada a ' + (guardian.contacts?.email || 'apoderado'), 'success');
    } catch (error) {
      console.error('Resend invitation error:', error);
      Components.showToast(error.message || 'Error al reenviar invitación', 'error');
    }
  };
